...
const Categories = ({setRoute, userId, showToast}) => {
  ...
  useEffect(()=>{ sb.from("categories").select("*").eq("user_id",userId).order("name").then(({data})=>{setCats(data||[]);setLoading(false);}); },[]);

  const add = async () => {
    if(!name.trim()) return;
    const {data,error} = await sb.from("categories").insert({name:name.trim(),user_id:userId}).select().single();
